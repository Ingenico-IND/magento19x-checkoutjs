...
/**
 * Ingenico Payment Standard Checkout Controller
 **/

require_once(Mage::getBaseDir() . '/app/code/community/Ingenico/Ingenico/Model/Standard.php');

class Ingenico_Ingenico_PaymentController extends Mage_Core_Controller_Front_Action
{
	public function getDebug()
	{
		return Mage::getSingleton('ingenico/config')->getDebug();
	}

	/**
	 * When a customer chooses Ingenico on Checkout/Payment page
	 *
	 */
	...
	{
		$this->loadLayout();
		$block = $this->getLayout()->createBlock('Mage_Core_Block_Template', 'test', array('template' => 'ingenico/ingenico.phtml'));
	...
	public function responseAction()
	{
		...
		if ($responseDetails[0] != '') {
			...
			Mage::log('Response: ' . $str, null, 'Ingenico_' . date("Ymd") . '.log', true);
			...
			if ($hashedString == $verificationHash) {
				...
				if ($responseDetails[0] == '0300' && $scallStatusCode == '0300') {
					...
					Mage::getSingleton('core/session')->addSuccess('Ingenico Message: ' . $txn_msg);
	...
	/**
	 * When a customer cancel payment from ingenico.
	 */
...

	{
		...
		if ($str) {
			...
			$hashableString = join('|', $responseDetails) . "|" . $key;
			$hashedString = hash('sha512',  $hashableString);
			Mage::log('Response S2S: ' . $str, null, 'Ingenico_' . date("Ymd") . '.log', true);
			$order = Mage::getModel('sales/order');
			$order->loadByIncrementId($orderId);
			$payment = $order->getPayment();
			$payment->setTransactionId($responseDetails[5]);
			...
			if ($responseDetails[0] == '0300' && $hashedString == $verificationHash) {
				//success
				$successstatus = Mage::getStoreConfig('payment/ingenico/order_status');
