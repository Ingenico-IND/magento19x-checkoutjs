...
/**
 * Worldline Standard Checkout Module
 **/

class Worldline_Worldline_Model_Standard extends Mage_Payment_Model_Method_Abstract
{
	protected $_code = 'Worldline';
	protected $_isInitializeNeeded      = true;
	protected $_formBlockType = 'Worldline/standard_form';
	...
	public function getRequest()
	{
		...
		$merchantMsg = Mage::getStoreConfig('payment/Worldline/merchantMsg');
		$txnType = Mage::getStoreConfig('payment/Worldline/txnType');
		$disclaimerMsg = Mage::getStoreConfig('payment/Worldline/disclaimerMsg');
		if (Mage::getStoreConfig('payment/Worldline/paymentModeOrder')) {
			$paymentModeOrder = Mage::getStoreConfig('payment/Worldline/paymentModeOrder');
		...
		$enableExpressPay = Mage::getStoreConfig('payment/Worldline/enableExpressPay');
		if ($enableExpressPay == '1') {
			$enableExpressPay = true;
		} else {
			$enableExpressPay = false;
		}
		$separateCardMode = Mage::getStoreConfig('payment/Worldline/separateCardMode');
		if ($separateCardMode == '1') {
			$separateCardMode = true;
		} else {
			$separateCardMode = false;
		}
		if (Mage::getStoreConfig('payment/Worldline/primary_color_code')) {
			$primary_color_code = Mage::getStoreConfig('payment/Worldline/primary_color_code');
		} else {
			$primary_color_code = '#3977b7';
		}
		if (Mage::getStoreConfig('payment/Worldline/secondary_color_code')) {
			$secondary_color_code = Mage::getStoreConfig('payment/Worldline/secondary_color_code');
		} else {
			$secondary_color_code = '#FFFFFF';
		}
		if (Mage::getStoreConfig('payment/Worldline/button_color_code_1')) {
			$button_color_code_1 = Mage::getStoreConfig('payment/Worldline/button_color_code_1');
		} else {
			$button_color_code_1 = '#1969bb';
		}
		if (Mage::getStoreConfig('payment/Worldline/button_color_code_2')) {
			$button_color_code_2 = Mage::getStoreConfig('payment/Worldline/button_color_code_2');
		} else {
			$button_color_code_2 = '#FFFFFF';
		}
		$logo_url = Mage::getStoreConfig('payment/Worldline/merchant_logo_url');
		...
		$enableNewWindowFlow = Mage::getStoreConfig('payment/Worldline/enableNewWindowFlow');
		if ($enableNewWindowFlow == '1') {
			$enableNewWindowFlow = true;
		} else {
			$enableNewWindowFlow = false;
		}
		$enableInstrumentDeRegistration = Mage::getStoreConfig('payment/Worldline/enableInstrumentDeRegistration');
		if ($enableInstrumentDeRegistration == '1' && $enableExpressPay == '1') {
			$enableInstrumentDeRegistration = true;
		} else {
			$enableInstrumentDeRegistration = false;
		}
		$hideSavedInstruments = Mage::getStoreConfig('payment/Worldline/hideSavedInstruments');
		if ($hideSavedInstruments == '1' && $enableExpressPay == '1') {
			$hideSavedInstruments = true;
		} else {
			$hideSavedInstruments = false;
		}
		$saveInstrument = Mage::getStoreConfig('payment/Worldline/saveInstrument');
		if ($saveInstrument == '1') {
			$saveInstrument = true;
		} else {
			$saveInstrument = false;
		}
		$embedPopup = Mage::getStoreConfig('payment/Worldline/embedPopup');
		if ($embedPopup == '1') {
			$embedPopup = '#Worldlinepayment';
		} else {
			$embedPopup = '';
		}
		$paymentmodes = Mage::getStoreConfig('payment/Worldline/paymentmodes');
		$MrctCode = Mage::getStoreConfig('payment/Worldline/Worldline_mercode');
		$MrctsCode = Mage::getStoreConfig('payment/Worldline/Worldline_scode');
		...
		$ReturnURL = Mage::getBaseUrl() . 'Worldline/payment/response';
		$restorequoteurl = Mage::getBaseUrl() . 'Worldline/payment/restorequote';
		$carturl = Mage::getBaseUrl() . 'checkout/cart';
		if (Mage::getStoreConfig('payment/Worldline/Worldline_url') == 'test') {
			$Amount = "1.0";
		} else {
		...
		Mage::log('Request: ' . $datastring, null, 'Worldline_' . date("Ymd") . '.log', true);
